process_results <- function(results){
  est.summaries <- filter(results,!name%in%c('Joint F','Trend')) %>% 
  ...
  test.summaries <- filter(results,name%in%c('Joint F','Trend')) %>% 
    # Compute the metrics relevant to individual effect estimates
    ...
    bind_rows(est.summaries %>% mutate(name="Individual") %>%
...
score_winners <- function(summaries){
  mod_test_perf <- summaries %>% 
  ...
  ## LAURA: fix this to work on test.summaries:
  mod_est_perf <- summaries %>% 
